...
仓颉语言实现的 MIDI 音乐编解码器，支持**乐谱记谱格式**与 **`.mid` 音频文件**之间的双向转换。
...
| **时值量化** | MIDI 音符的 tick 时长会被匹配到最接近的标准/附点/三连音时值（15% 容差），非标准时值用 `T<ticks>` 表示 |
| **和弦时值统一** | 同时开始但时值不同的音符被合并为和弦，使用最短时值 |
...
- **音符数量完全一致**：解码 → 重编码后，NoteOn 事件总数与原始文件完全相同
- **音高集合完整保留**：所有原始音高在重编码后均被保留
- **和弦保真**：同时发声的音符始终保持为和弦，不会被拆分为顺序播放
- **智能 IOI 修正**：仅在能产生更好标准时值匹配时才替换原始时值，不破坏原始节奏
- **三连音识别**：自动识别并输出三连音时值（`4t`、`8t` 等），减少 `T<ticks>` 原始值
- **控制事件保留**：CC 和 PitchBend 事件的数量和参数完整保留
...
所有标准时值均支持附点（`.`，时值 ×1.5）和三连音（`t`，时值 ×2/3）后缀：

| 后缀 | 说明 | 示例 | ticks (TPQ=480) |
|------|------|------|-----------------|
| `.` | 附点 | `4.` = 附点四分音符 | 720 |
| `.` | 附点 | `8.` = 附点八分音符 | 360 |
| `t` | 三连音 | `4t` = 三连音四分音符 | 320 |
| `t` | 三连音 | `8t` = 三连音八分音符 | 160 |
