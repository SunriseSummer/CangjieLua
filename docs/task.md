...
**特性描述**：支持浮点数的科学记数法表示，如 `1.5e10`、`2.0E-3`。仓颉语言支持此特性。
...
**期望效果**：
- 词法分析器能识别 `e`/`E` 后跟可选符号和数字的科学记数法
- 正确转换为浮点数值

**实现提示**：
1. **词法分析器**（`lexer.cj`）：在 `readNumber()` 中，读取数字部分后检查下一个字符是否为 `e` 或 `E`
2. 如果是，继续读取可选的 `+`/`-` 符号和指数部分的数字
3. 将整个字符串（如 `"1.5e10"`）作为浮点数解析
4. 生成与普通浮点数相同的 `NUMBER` token
...
**特性描述**：支持仓颉语言中的位与（`&`）和位或（`|`）运算符。结合任务 14 的按位取反 `~`，构成完整的基础位运算支持。
...
**特性描述**：支持获取数组的元素个数。仓颉语言中通过 `.size` 属性获取集合的长度，此任务将 `.size` 扩展到数组类型。
...
**实现提示**：
1. 复用任务 15 的 `.size` 属性解析逻辑（`PropertyAccess` AST 节点）
2. **代码生成器**：对 `.size` 属性，统一生成 `LEN` 指令（OpCode 52），不区分字符串和 table
3. Lua 的 `#` 运算符对 table 返回序列长度（连续整数索引的元素数），与数组语义一致
...
**特性描述**：支持通过索引访问字符串中的单个字符，返回该位置的单字符字符串。
...
**期望效果**：
- `str[index]` 返回指定位置的字符（以单字符字符串形式）
- 索引从 0 开始（编译时转换为 Lua 的 1-based）

**实现提示**：
1. 复用任务 18（数组索引访问）的 `IndexAccess` AST 节点
2. **代码生成器**：对字符串索引，可以在运行时调用 Lua 的 `string.sub` 函数
3. 将 0-based 索引转换为 Lua 的 1-based：`string.sub(s, i+1, i+1)` 获取单个字符
4. 也可以注册一个全局辅助函数 `charAt` 来实现
